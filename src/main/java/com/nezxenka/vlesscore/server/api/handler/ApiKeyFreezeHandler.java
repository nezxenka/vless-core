...
public class ApiKeyFreezeHandler implements HttpHandler {

    private static final Logger log = LoggerFactory.getLogger(ApiKeyFreezeHandler.class);

    private final AppConfig config;
    private final TokenDao tokenDao;
    private final ApiAuthFilter auth;

    public ApiKeyFreezeHandler(AppConfig config, TokenDao tokenDao) {
        this.config = config;
    ...
    public void handle(HttpExchange ex) throws IOException {
        ...
        try {
            ...
            ApiResponseBuilder.sendJson(ex, 200, "{\"success\":true,\"status\":\"BLOCKED\"}");
            log.info("[API] Key frozen: {}",
